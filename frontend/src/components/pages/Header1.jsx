...
import '../styles/Header1.css'; // CSS for styling the header
...
const Header1 = () => {
  ...
  return (
    <header className={`head_header ${showHeader ? 'head_header-visible' : ''}`}>
      <div className="head_go-back-container">
        <button className="head_go-back" onClick={handleGoBack}>Go Back</button>
      </div>
      <div className="head_welcome-message">
      WELCOME TO ARDUR TECHNOLOGY
      </div>
      <div className="head_employee-logout">
        <span className="head_employee-id">{`Login ID: ${employeeId}`}</span>
        <button className="head_logout" onClick={handleLogout}>Logout</button>
      </div>
    </header>
  );
};
